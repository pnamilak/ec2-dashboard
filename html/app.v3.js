(function(){
  ...
  function render(){
    ...
    list.forEach(x=>{const s=$('start-'+x.id), t=$('stop-'+x.id), d=$('det-'+x.id);
      if(s) s.onclick=()=>instAction(x.id,'start'); if(t) t.onclick=()=>instAction(x.id,'stop'); if(d) d.onclick=()=>openDetails(x);});
  }

  function pill(st){const c=st==='running'?'style="border-color:#14532d;background:#0a1f12;color:#a7f3d0"':
                    st==='stopped'?'style="border-color:#4b1d1d;background:#1a0b0b;color:#fecaca"':''; return `<span class="pill" ${c}>${st}</span>`;}
  function card(x){const canStart=x.state==='stopped', canStop=x.state==='running';
    return `<article class="card"><header class="head">
      <div class="name" title="${x.name}">${x.name}</div><span class="pill">${x.env}</span>${pill(x.state)}<div class="grow"></div>
      <button class="btn ghost" id="det-${x.id}">Details</button>${canStart?`<button class="btn primary" id="start-${x.id}">Start</button>`:''}
      ${canStop?`<button class="btn" id="stop-${x.id}">Stop</button>`:''}</header>
      <div class="meta"><div class="kv">ID: ${x.id}</div><div class="kv">Platform: ${x.platform}</div>
      <div class="kv">Private IP: ${x.privateIp||'-'}</div><div class="kv">Public IP: ${x.publicIp||'-'}</div></div></article>`;}

  async function instAction(id, action){ try{ await api('/instances',{method:'POST',body:JSON.stringify({action,instanceId:id})}); await loadInstances(false);}catch(e){alert('Action failed: '+e.message);} }

  // Details modal ... (same JS as your current build; omitted here for brevity)
  // Keep your existing details modal functions here.
