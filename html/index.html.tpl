...
<head>
  ...
  <style>
    :root { --bg:#0b1220;--panel:#121b2e;--muted:#a9b4c6;--text:#e7eefc;--danger:#ff6b6b;--ok:#63e6be;--btn:linear-gradient(90deg,#5aa8ff,#79f8d6) }
    html,body{margin:0;height:100%;background:var(--bg);color:var(--text);font:16px/1.4 system-ui,-apple-system,Segoe UI,Roboto,Ubuntu}
    .wrap{min-height:100%;display:grid;place-items:center;padding:24px}
    .card{width:min(760px,92vw);background:rgba(18,27,46,.92);border-radius:16px;box-shadow:0 20px 60px rgba(0,0,0,.45);padding:28px}
    h1{margin:0 0 14px;font-size:22px}
    p.sub{margin:0 0 20px;color:var(--muted)}
    .row{display:flex;gap:12px;flex-wrap:wrap}.row>*{flex:1 1 260px}
    label{display:block;margin:16px 0 8px;color:var(--muted);font-size:13px}
    input{width:100%;height:44px;border-radius:10px;border:1px solid #1e2942;background:#0e1627;color:var(--text);padding:0 12px;outline:none}
    input:focus{border-color:#2f8fff;box-shadow:0 0 0 3px rgba(47,143,255,.15)}
    button{height:44px;border:0;border-radius:10px;padding:0 16px;cursor:pointer;color:#081018;font-weight:600}
    .btnPrimary{background:var(--btn);min-width:140px}.btnGhost{background:#1a2440;color:#d9e7ff}
    .msg{min-height:22px;margin-top:10px;font-size:13px}.msg.ok{color:var(--ok)}.msg.err{color:var(--danger)}
...
<body>
  <div class="wrap">
    <div class="card">
      ...
      <div class="foot">After successful verification you’ll be redirected to the credentials page.</div>
    </div>
  </div>
  ...
  (function () {
    ...
    const setMsg  = (t,c)=>{msgEl.className='msg '+(c||'');msgEl.textContent=t||''};
    ...
    reqBtn.onclick = async () => {
      ...
      try { reqBtn.disabled=true; await post('/request-otp',{email}); setMsg('otp_sent','ok'); }
      catch(e){ setMsg(e.message||'send_failed','err'); }
    ...
    verBtn.onclick = async () => {
      ...
      try {
        ...
        // 10-min backup: localStorage (auto-rehydrated by login.js)
        try { localStorage.setItem('otp_bundle', JSON.stringify({t:code,e:email,ts})); } catch(_){}
