...
resource "aws_s3_bucket" "frontend" { bucket = local.resolved_bucket_name  force_destroy = true }
...
data "archive_file" "lambda_zip" {
  ...
  source { content = file("${locals.lambda_dir}/handler.py");    filename = "handler.py" }
  source { content = file("${locals.lambda_dir}/authorizer.py"); filename = "authorizer.py" }
}
...
resource "aws_iam_policy" "ec2_control_policy" {
  ...
  policy = jsonencode({Version="2012-10-17", Statement=[{Effect="Allow", Action=["ec2:DescribeInstances","ec2:StartInstances","ec2:StopInstances","ec2:RebootInstances"], Resource="*"}]})
}
...
resource "aws_iam_policy" "lambda_ssm_commands" {
  ...
  policy = jsonencode({
    ...
    Statement=[{
      Effect="Allow",
      Action=["ssm:SendCommand","ssm:GetCommandInvocation","ssm:ListCommands","ssm:ListCommandInvocations","ssm:CancelCommand","ssm:DescribeInstanceInformation"],
      Resource="*"
    }]
  })
}
...
resource "aws_iam_role_policy_attachment" "ec2_ssm_core" { role = aws_iam_role.ec2_ssm_role.name  policy_arn = "arn:aws:iam::aws:policy/AmazonSSMManagedInstanceCore" }
resource "aws_iam_instance_profile" "ec2_ssm_profile" { name_prefix = "ec2-ssm-instance-profile-"  role = aws_iam_role.ec2_ssm_role.name }

data "aws_instances" "ssm_attach_targets" {
  filter { name = "instance-state-name" values = var.instance_state_filter }
  dynamic "filter" {
    for_each = var.target_tag_selector
    content { name = "tag:${filter.key}" values = [filter.value] }
  }
}
...
resource "null_resource" "associate_ssm_profile" {
  ...
  triggers = { instance_id = each.value  profile = aws_iam_instance_profile.ec2_ssm_profile.name }
...
# ------------ HTML + versioned JS (cache-busting) -------------
locals { app_js_path = "${local.web_dir}/app.v3.js"  app_js_md5 = filemd5(local.app_js_path) }

data "template_file" "html" {
  template = file("${local.web_dir}/index.html.tpl")
  vars = { api_url = aws_apigatewayv2_stage.default.invoke_url  js_ver = local.app_js_md5 }
}
